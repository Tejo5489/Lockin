...
// FEATURE: Calculate Fire Streaks for Habits
function calculateStreak(habit) {
    ...
    // Carry over last week's momentum if we are perfectly consistent this week
    if (streak === todayIndex + 1 && habit.lastWeekCount > 0) {
        streak += habit.lastWeekCount;
    }
    return streak;
}
...
function renderHabits() {
    ...
    habits.forEach(habit => {
        ...
        // Display Fire Streak if active
        const streak = calculateStreak(habit);
        const streakHTML = streak > 1 ? `<span class="ml-3 text-brand-amber text-sm font-black bg-orange-100 px-2 py-1 rounded-md" title="Current Streak">🔥 ${streak}</span>` : '';

        list.innerHTML += `
            <tr class="border-b-2 border-navy border-opacity-20 hover:bg-white transition-colors">
                <td class="py-4 font-bold text-navy text-lg flex items-center h-full min-h-[72px]">${habit.title} ${streakHTML}</td>
                ${checkboxesHTML}
...
// FEATURE UPGRADE: Rich Visual Reports
function generateAIReport() {
    ...
    let percentChange = 0;
    let trendStrPlain = "";
    let trendColorClass = "text-navy-light";
    let trendIcon = "";

    if (lastWeekHabits > 0) {
        percentChange = Math.round(((currentWeekHabits - lastWeekHabits) / lastWeekHabits) * 100);
        if (percentChange >= 0) {
            trendStrPlain = `(+${percentChange}% vs last week 📈)`;
            trendColorClass = "text-brand-green";
            trendIcon = '<i class="ph ph-trend-up"></i>';
        } else {
            trendStrPlain = `(${percentChange}% vs last week 📉)`;
            trendColorClass = "text-brand-red";
            trendIcon = '<i class="ph ph-trend-down"></i>';
        }
    } else if (lastWeekHabits === 0 && currentWeekHabits > 0) {
        trendStrPlain = `(+100% vs last week 📈)`;
        trendColorClass = "text-brand-green";
        trendIcon = '<i class="ph ph-trend-up"></i>';
        percentChange = 100;
    } else if (lastWeekHabits === 0 && currentWeekHabits === 0) {
        trendStrPlain = `(0% vs last week 📉)`;
    ...
    // 1. GENERATE PLAIN TEXT (For the actual file download)
    const plainReport = `🔥 LockIn | Execution Report\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n🎯 Objectives Secured: ${completedTasks}\n✅ Habits Maintained: ${currentWeekHabits} ${trendStrPlain}\n⚡ AI Insight: ${insight}\n\nOutwork everyone.\n#LockedIn #DeepWork`;
    document.getElementById('reportText').value = plainReport;

    // 2. GENERATE RICH HTML (For the visual modal preview)
    const visualHTML = `
        <div class="flex items-center justify-between border-b-2 border-navy border-opacity-20 pb-3">
            <span class="font-display font-black text-navy uppercase tracking-widest flex items-center gap-2">
                <i class="ph ph-fire text-brand-amber text-xl"></i> Protocol Report
            </span>
            <span class="text-[10px] font-bold text-navy-light uppercase bg-beige px-2 py-1 rounded">Official</span>
        </div>
        
        <div class="grid grid-cols-2 gap-4">
            <div class="bg-white border-2 border-navy rounded-lg p-3 shadow-[2px_2px_0px_0px_#0F172A]">
                <p class="text-[10px] font-black uppercase tracking-widest text-brand-blue mb-1">Objectives</p>
                <div class="flex items-end gap-2">
                    <span class="text-3xl font-mono font-black text-navy">${completedTasks}</span>
                    <i class="ph ph-target text-xl text-navy-light mb-1"></i>
                </div>
            </div>
            <div class="bg-white border-2 border-navy rounded-lg p-3 shadow-[2px_2px_0px_0px_#0F172A]">
                <p class="text-[10px] font-black uppercase tracking-widest text-brand-green mb-1">Habits</p>
                <div class="flex items-end justify-between">
                    <span class="text-3xl font-mono font-black text-navy">${currentWeekHabits}</span>
                    <span class="text-sm font-bold ${trendColorClass} flex items-center gap-1 mb-1">
                        ${percentChange > 0 ? '+'+percentChange : percentChange}% ${trendIcon}
                    </span>
                </div>
            </div>
        </div>

        <div class="bg-beige-dark border-2 border-navy rounded-lg p-4 shadow-[2px_2px_0px_0px_#0F172A] mt-2">
            <p class="text-[10px] font-black uppercase tracking-widest text-navy-light mb-2 flex items-center gap-1"><i class="ph ph-brain text-brand-blue"></i> AI Insight</p>
            <p class="text-sm font-bold text-navy leading-tight italic">"${insight}"</p>
        </div>
    `;
    
    document.getElementById('reportVisual').innerHTML = visualHTML;
    ...
    actionBtn.onclick = function() {
        ...
        this.classList.replace('neo-btn-blue', 'bg-brand-green');
        this.classList.add('text-white');
        
        setTimeout(() => { 
            this.innerHTML = '<i class="ph ph-download-simple"></i> Download Report'; 
            this.classList.replace('bg-brand-green', 'neo-btn-blue');
        }, 2000);
    };
}
...
// ============================================================================
// FEATURE: PREMIUM TOOLSET INJECTION (Pomodoro & Ambient Audio)
// ============================================================================
...
function toggleAudio(type) {
    ...
    if (currentNoise === type) {
        ...
        currentNoise = null;
        document.getElementById('btnWhiteNoise').className = 'neo-btn neo-btn-white px-3 py-1 text-xs rounded';
        document.getElementById('btnBrownNoise').className = 'neo-btn neo-btn-white px-3 py-1 text-xs rounded';
        return;
    }
    ...
    document.getElementById('btnWhiteNoise').className = `neo-btn px-3 py-1 text-xs rounded ${type === 'white' ? 'bg-navy text-white' : 'neo-btn-white'}`;
    document.getElementById('btnBrownNoise').className = `neo-btn px-3 py-1 text-xs rounded ${type === 'brown' ? 'bg-navy text-white' : 'neo-btn-white'}`;
}
    ...
    if (pomoActive) {
        ...
        if (!pomoInterval) {
            pomoInterval = setInterval(() => {
                ...
                pomoTime--;
                display.innerText = formatPomo(pomoTime);
                ...
                if (pomoTime <= 0) {
                    ...
                    if (pomoMode === 'focus') {
                        pomoMode = 'break';
                        pomoTime = 5 * 60; 
                        display.classList.replace('text-brand-blue', 'text-brand-green');
                        alert("Deep Work Block Complete! Taking a 5-minute breather.");
                    } else {
                        pomoMode = 'focus';
                        pomoTime = 25 * 60; 
                        display.classList.replace('text-brand-green', 'text-brand-blue');
                        alert("Break Over. Lock back in.");
                    }
                }
            }, 1000);
        }
...
function injectPremiumUI() {
    ...
    panel.className = 'fixed bottom-6 right-6 flex flex-col gap-4 z-40';
    panel.innerHTML = `
        <div class="neo-panel p-5 bg-white flex flex-col gap-4 shadow-[8px_8px_0px_0px_#0F172A] border-[3px] border-navy">
            <h4 class="text-xs font-black uppercase tracking-widest text-navy border-b-[3px] border-navy pb-2 font-display">Premium Toolset</h4>
            
            <div class="flex items-center justify-between gap-6">
                <span class="text-sm font-bold text-navy flex items-center gap-2"><i class="ph ph-headphones text-xl text-brand-amber"></i> Audio</span>
                <div class="flex gap-2">
                    <button id="btnWhiteNoise" onclick="toggleAudio('white')" class="neo-btn neo-btn-white px-3 py-1 text-xs rounded">White</button>
                    <button id="btnBrownNoise" onclick="toggleAudio('brown')" class="neo-btn neo-btn-white px-3 py-1 text-xs rounded">Brown</button>
                </div>
            </div>
            
            <div class="flex items-center justify-between gap-6">
                <span class="text-sm font-bold text-navy flex items-center gap-2"><i class="ph ph-timer text-xl text-brand-red"></i> Pomodoro</span>
                <div class="flex items-center gap-3">
                    <span id="pomoDisplay" class="font-mono font-black text-lg text-brand-blue w-14 text-center">25:00</span>
                    <button id="btnPomo" onclick="togglePomodoro()" class="neo-btn neo-btn-white px-3 py-1 text-xs rounded flex items-center gap-1">
